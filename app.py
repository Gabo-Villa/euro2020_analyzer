...
st.sidebar.markdown(
    """[<img src='data:image/png;base64,{}' class='img-fluid' width=140 height=140>](https://www.uefa.com/uefaeuro/history/seasons/2020/)""".format(
        img_to_bytes("euro_2020.png")
    ),
    unsafe_allow_html=True,
)

categories = st.sidebar.selectbox("Select the category:", ["Matches", "Players"])
...
if categories == "Matches":
    ...
    competition_stage = st.sidebar.selectbox(
        "Select a stage of the tournament:", stages_unique
    )
    ...
    if visualizations == "Lineups":
        ...
        st.sidebar.write(
            "[What are Expected Goals (xG)?](https://theanalyst.com/eu/2021/07/what-are-expected-goals-xg/)"
        )
        ...
        st.sidebar.write(
            "[What is Expected Threat (xT)?](https://karun.in/blog/expected-threat.html)"
        )
    ...
    tables = st.sidebar.selectbox(
        "Select a table:", ["Goals", "Assists", "Red cards", "Saves"]
    )
...
with st.sidebar.expander("About the project"):
    st.write(
        "Data visualization app to analyze the Euro 2020. All the data is provided by [Statsbomb](https://statsbomb.com/)."
    )
