...
def goals(match_id):
    ...
    lineup.drop(
        ["player_name", "match_id", "team_id", "country_id", "country_name"],
        axis=1,
        inplace=True,
    )
    ...
    if len(df_goals) == 0:
        ...
        goals = st.sidebar.selectbox(
            "Select a goal:",
            df_goals["player_nickname"].loc[df_goals["match_id"] == match_id]
            + " "
            + df_goals["minute"].astype(str).loc[df_goals["match_id"] == match_id]
            + ":"
            + df_goals["second"].astype(str).loc[df_goals["match_id"] == match_id],
        )

        shot_id = df_goals["id"].loc[
            df_goals["player_nickname"]
            + " "
            + df_goals["minute"].astype(str)
            + ":"
            + df_goals["second"].astype(str)
            == goals
        ]
        ...
        df_team2_goalkeeper = df_freeze_frame.loc[
            df_freeze_frame["team_name"] == team2
        ][df_freeze_frame["position_name"] == "Goalkeeper"]
        df_team2_other = df_freeze_frame.loc[df_freeze_frame["team_name"] == team2][
            df_freeze_frame["position_name"] != "Goalkeeper"
        ]
        ...
        # Plot the players
        sc1 = pitch.scatter(
            df_team1["x"], df_team1["y"], s=700, c="#ba495c", label=team1, ax=axs
        )
        sc2 = pitch.scatter(
            df_team2_other["x"],
            df_team2_other["y"],
        ...
        sc4 = pitch.scatter(
            df_team2_goalkeeper["x"],
            df_team2_goalkeeper["y"],
            s=700,
            ax=axs,
            c="#5ba965",
            label="Goalkeeper",
        ...
        for i, label in enumerate(df_freeze_frame["jersey_number"]):
            pitch.annotate(
                label,
                (df_freeze_frame["x"][i], df_freeze_frame["y"][i]),
                va="center",
                ha="center",
                color="white",
                fontsize=15,
        ...
        axs.set_title(
            f"{shooter} {minute_goal}:{second_goal}\nBody part: {body_part_goal}\n{play_pattern_goal}\nxG: {xG_goal}",
            ha="center",
            fontsize=25,
        )
