...
def players_statistics(tables):
    ...
    datacoord_to_normalizedfigurecoord = lambda x: figurecoord_to_normalizedfigurecoord(
        datacoord_to_figurecoord(x)
    )
    # Flag axes
    ...
    for i in range(nrows):
        for index_row, column in enumerate(columns):
            ...
            if column in bold_columns:
                ...
                weight = "normal"
            if column == "Team_img":
                ax_coords = datacoord_to_normalizedfigurecoord([1.75, i + 0.25])
    ...
    ax.fill_between(x=[5.5, 6.5], y1=nrows, y2=0, color=color, alpha=0.5, ec="None")
