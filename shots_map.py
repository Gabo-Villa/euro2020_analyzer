...
def shots_map(match_id):
    ...
    team1_shots_count = (
        df_team1_goals.count()[0] + df_team1_non_goals.count()[0] - team1_owngoals_count
    )
    ...
    team2_shots_count = (
        df_team2_goals.count()[0] + df_team2_non_goals.count()[0] - team2_owngoals_count
    )
    team2_xG = round(df_team2.shot_statsbomb_xg.sum(), 2)

    # Plot team 1non-goal shots
    team1_non_goals = pitch.scatter(
        df_team1_non_goals.x,
        df_team1_non_goals.y,
    ...
    team2_non_goals = pitch.scatter(
        df_team2_non_goals.x,
        df_team2_non_goals.y,
    ...
    ax.add_artist(ada)
    txt1 = ax.text(
        x=26, y=4.5, s="xG Value", color="k", ha="center", va="center", fontsize=30
    )
    txt2 = ax.text(
        x=19, y=11.5, s="Goal", color="k", ha="center", va="center", fontsize=30
    )
    ...
    txt5 = ax.text(
        x=60,
        y=-12,
        s=f"{team1_goals_count}     Goals     {team2_goals_count}",
        ha="center",
        va="center",
        fontsize=30,
    )
    txt6 = ax.text(
        x=60,
        y=-7,
        s=f"{team1_shots_count}     Shots     {team2_shots_count}",
        ha="center",
        va="center",
        fontsize=30,
    )
