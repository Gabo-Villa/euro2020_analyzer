...
def xG_flowchart(match_id):
    ...
    df_shots = df_no_penalties[
        (df.type_name == "Shot") | (df.type_name == "Own Goal For")
    ]
    # Replace the outcome name of the own goals to be a goal
    df_shots["outcome_name"] = df_shots["outcome_name"].apply(
        lambda x: str(x).replace("nan", "Goal")
    )
    ...
    fig, ax = plt.subplots(
        figsize=(14, 11), constrained_layout=True, tight_layout=False
    )
    fig.set_facecolor("w")
    ...
    # Plot the xG values
    ax.step(
        x=team1_min, y=team1_cumulative_xG, linewidth=4, where="post", color="#ba495c"
    )
    ax.step(
        x=team2_min, y=team2_cumulative_xG, linewidth=4, where="post", color="#697cd4"
    )
    ...
    # Legend
    team1_patch = mpatches.Patch(
        color="#ba495c", label=f"{team1}: {str(round(team1_cumulative_xG[-1], 2))} xG"
    )
    team2_patch = mpatches.Patch(
        color="#697cd4", label=f"{team2}: {str(round(team2_cumulative_xG[-1], 2))} xG"
    )
    goal_marker = mlines.Line2D(
        [], [], color="w", marker="*", markersize=40, markeredgecolor="k", label="Goal"
    )
    ax.legend(
        handles=[team1_patch, team2_patch, goal_marker], fontsize=30, frameon=False
    )
