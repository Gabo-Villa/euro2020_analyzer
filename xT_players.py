...
def xT_players(match_id):
    ...
    df_xT = df_event.loc[
        (df_event["type_name"] == "Pass") | (df_event["type_name"] == "Carry")
    ...
    df_xT["start_zone_value"] = df_xT[["x1_bin", "y1_bin"]].apply(
        lambda x: xT_grid[x[1]][x[0]], axis=1
    )
    df_xT["end_zone_value"] = df_xT[["x2_bin", "y2_bin"]].apply(
        lambda x: xT_grid[x[1]][x[0]], axis=1
    )
    df_xT["xT"] = df_xT["end_zone_value"] - df_xT["start_zone_value"]
    df_xT["xT_player"] = df_xT["player_nickname"].apply(
        lambda x: df_xT["xT"].loc[df_xT["player_nickname"] == x].sum()
    )
    ...
    df_plot = pd.DataFrame({"player": player_list, "xT": xT_list, "team": team_list})
